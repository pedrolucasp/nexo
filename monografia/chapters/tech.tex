...
máquinas de desenvolvimento e o servidor de produção. O uso de
\textit{healthchecks} em conjunto com a diretiva \texttt{depends\_on} e a
condição \texttt{service\_healthy} assegura que a API só seja iniciada após o
banco de dados e o Valkey estarem de fato prontos para aceitar conexões,
evitando falhas de inicialização por condição de corrida (race condition), como ilustrado no
